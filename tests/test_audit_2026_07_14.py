...
class GenActionIndexCoversPackagesAndAliasesTests(unittest.TestCase):
    ...
    def test_alias_form_resolves_to_target_factory(self):
        # #45's alias form, now handled by the shared scanner
        # (tools/registration_scan.py — the 2026-07-21 audit consolidated the
        # regex copies that used to live in gen_action_index.py): the alias
        # must land on the TARGET's factory symbol, where the old single regex
        # captured the bare symbol "actions" (a `?` location).
        import importlib.util
        spec = importlib.util.spec_from_file_location(
            "registration_scan",
            os.path.join(_PROJECT, "tools", "registration_scan.py"))
        rs = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(rs)
        regs = rs.scan_registrations(
            "def register(actions):\n"
            "    actions['schedule_recurring'] = _make_recurring(scheduler)\n"
            "    actions['schedule_cron'] = actions['schedule_recurring']\n")
        self.assertEqual(regs["schedule_cron"].symbol, "_make_recurring")
...
class SwitchLlmTagIsAuthoritativeTests(unittest.TestCase):
    ...
    def _bc(self, installed):
        ...
        # (exact/sibling-aware, returns the CONCRETE tag or None). Identity =
        # "the requested tag is installed verbatim".
        bc._ollama_resolve_model.side_effect = \
            (lambda t: t) if installed else (lambda t: None)
        bc._get_local_llm_model.return_value = "gemma4:12b"
        return bc
